...
$plan = null;
$planKey = $clientePlan['plan'] ?? null;
if ($planKey && isset($catalogo[$planKey])) {
    $plan = $catalogo[$planKey];
}

$nombre = $ficha['nombre'] ?: $slug;
$fecha = date('d/m/Y');
$precioCustom = $clientePlan['precio_custom'] ?? null;
$fee = $ficha['info']['Fee'] ?? $ficha['info']['Fee actual'] ?? $precioCustom ?? 'Por definir';
?>
<!DOCTYPE html>
<html lang="es">
<head>
...
.client-banner .plan-badge { background: #f97316; color: #fff; padding: 6px 16px; border-radius: 20px; font-size: 13px; font-weight: 700; }
...
<div class="header">
    <div class="header-top">
        ...
        <div class="header-right">
            <p>Documento de Alcances del Servicio</p>
            <p><?= $fecha ?></p>
        </div>
    </div>
...
<p class="intro">
    Estimado/a cliente, a continuación se detallan los alcances del servicio contratado con Facand. Este documento describe qué incluye su plan, las herramientas que configuramos y gestionamos, y las responsabilidades de cada parte para asegurar los mejores resultados.
</p>
...
<!-- Qué incluye -->
<div class="section">
    <div class="section-title">Qué incluye su servicio</div>
    <ul class="check-list">
...
<div class="grid-2">
    <!-- No incluye -->
    <div class="section">
        <div class="section-title">No incluido</div>
        <ul class="cross-list">
            <?php foreach ($plan['no_incluye'] as $item): ?>
            <li><?= htmlspecialchars($item) ?></li>
            <?php endforeach; ?>
        </ul>
    </div>
    ...
    <div class="section">
        ...
        <div class="tool-tags">
            <?php foreach ($plan['herramientas'] as $h): ?>
...
<!-- Responsabilidades -->
<div class="section">
    <div class="section-title">Responsabilidades del cliente</div>
    <ul class="resp-list">
        <?php foreach ($plan['responsabilidades'] as $r): ?>
...
<div class="note-box">
    <strong>Material audiovisual:</strong> Todo el material fotográfico y de video es responsabilidad exclusiva del cliente. Facand utiliza el material entregado para la creación de anuncios, creatividades y contenido web. La calidad del material impacta directamente en el rendimiento de las campañas. Facand puede orientar sobre formatos, duración y estilo requerido.
</div>
...
<div class="footer">
    <strong>FACAND</strong> — Agencia Digital<br>
